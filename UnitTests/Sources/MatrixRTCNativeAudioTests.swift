...
final class MatrixRTCNativeAudioTests {
    ...
    func liveKitKeyProviderMatchesElementCallFrameCrypto() throws {
        ...
        #expect(handoff.contains("shouldNegotiateCapabilities: true"))
        #expect(handoff.contains("(widgetDriver as? ElementCallWidgetDriver)?.stop()"))
        #expect(handoff.contains("enableOutgoingToDeviceForCall"))
        
        let clientProxy = try Self.source(named: "ElementX/Sources/Services/Client/ClientProxy.swift")
        #expect(clientProxy.contains("func enableOutgoingToDeviceForCall() async"))
        #expect(clientProxy.contains("await client.enableAllSendQueues(enable: true)"))
        
        let widgetDriver = try Self.source(named: "ElementX/Sources/Services/ElementCall/ElementCallWidgetDriver.swift")
        #expect(widgetDriver.contains("native-audio-widget-stop"))
        #expect(widgetDriver.contains("func stop()"))
