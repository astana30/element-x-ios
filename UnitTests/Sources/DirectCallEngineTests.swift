...
final class NativeIncomingCallLifecycleContractTests {
    ...
    func receiverLeaseDoesNotMarkSenderVisibilityTerminalBeforeBothLocalAudioGate() throws {
        ...
        #expect(adapterSource.contains("recordReceiverConnectedSessionLeaseReleaseBlockedForReceiverPublishPending"))
        #expect(adapterSource.contains("let receiverPublishPending = !explicitTeardown"))
    ...
    func remoteAudioLivenessDiagnosticsFieldsAndDefaultsAreRedacted() throws {
        ...
        #expect(adapterSource.contains("sender_local_audio_after_join_required_bucket=\\(senderLocalAudioAfterJoinRequiredBucket)"))
        #expect(adapterSource.contains("sender_local_audio_after_join_attempt_count_bucket=\\(senderLocalAudioAfterJoinAttemptCountBucket)"))
        #expect(adapterSource.contains("sender_local_audio_after_join_result_bucket=\\(senderLocalAudioAfterJoinResultBucket)"))
    ...
    func senderRuntimeLiveKitJoinBridgeUsesRuntimeCallbacksNotQueryOutcomes() throws {
        ...
        #expect(adapterSource.contains("localAudioPublish = await publishSenderLocalAudioAfterRuntimeJoin(client: client)"))
        #expect(adapterSource.contains("private static func publishSenderLocalAudioAfterRuntimeJoin(client: DirectCallLiveKitClientProtocol)"))
        #expect(adapterSource.contains("summary.markRuntime(result)"))
        #expect(adapterSource.contains("summary.markLocalAudioPublish(localAudioPublish.result, attemptCount: localAudioPublish.attemptCount)"))
        ...
        #expect(adapterSource.contains("mutating func markLocalAudioPublish(_ result: Result<Void, DirectCallMediaError>, attemptCount: Int)"))
        #expect(adapterSource.contains("senderLocalAudioAfterJoinRequiredBucket = \"required_redacted\""))
        #expect(adapterSource.contains("senderLocalAudioAfterJoinResultBucket = \"success_redacted\""))
        #expect(adapterSource.contains("senderLocalAudioAfterJoinResultBucket = \"failed_redacted\""))
    ...
    func receiverAudioPublishGateIsDebugOnlyAndRequiresConnectedAnswerBeforeMicrophone() throws {
        ...
        #expect(source.contains("summary.receiverLiveKitJoinStateLatchBucket == \"available_redacted\""))
        ...
        #expect(source.contains("scheduleReceiverAudioPublishDispatchReplay()"))
        #expect(source.contains("private static func scheduleReceiverAudioPublishDispatchReplay(attempt: Int = 0)"))
        #expect(source.contains("private static func scheduleReceiverAudioPublishDispatchReplayIfReady()"))
        #expect(source.contains("scheduleReceiverAudioPublishDispatchReplayIfReady()"))
        #expect(source.contains("!receiverAudioPublishTriggerConsumed"))
        #expect(source.contains("try? await Task.sleep(nanoseconds: 300_000_000)"))
