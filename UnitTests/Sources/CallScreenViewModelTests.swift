...
extension CallScreenViewModelTests {
    ...
    func appStoreLinePresentsIncomingCallsFromMatrixRTCPresence() throws {
        ...
        #expect(appCoordinator.contains("matrixrtc-presence-incoming capability-retry membership-timeout keep-call-on-dispatch-fail wait-membership-before-prepare http-status-log"))
        ...
        #expect(dispatchClient.contains("Production dispatch HTTP failed path=\\(path) status=\\(response.statusCode) error=\\(error)"))
