...
extension ProcessInfo {
    ...
    static var isNativeDirectCallInternalPilotActivationDryRunEnabled: Bool {
        #if DEBUG
        isNativeDirectCallInternalPilotActivationDryRunEnabled(environment: processInfo.environment)
        #else
        false
        #endif
    }
...
extension ProcessInfo {
    ...
    static func isNativeDirectCallInternalPilotActivationDryRunEnabled(environment: [String: String]) -> Bool {
        isNativeDirectCallDiagnosticIntegrationCommandsEnabled(environment: environment) &&
            environment["NATIVE_DIRECT_CALL_INTERNAL_PILOT_ACTIVATION_DRY_RUN_ENABLED"] == "1"
    }
