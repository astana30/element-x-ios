...
class AppCoordinator: AppCoordinatorProtocol, AuthenticationFlowCoordinatorDelegate, NotificationManagerDelegate, SecureWindowManagerDelegate {
    ...
    init(appDelegate: AppDelegate) {
        ...
        MXLog.info("App Store line build: callkit-end-on-hangup unstick-handset audio-only no-video-button outgoing-hangup-callkit answer-on-callkit-activate earpiece-voicechat no-audio-session-loop reassert-voicechat-category matrixrtc-presence-incoming capability-retry membership-timeout keep-call-on-dispatch-fail wait-membership-before-prepare http-status-log opaque-call-handle presence-requires-remote keep-audio-overlay dispatch-401-retry leave-webrtc-audio-session no-native-category virtual-earpiece-default pushkit-token-hex sandbox-apns-activation message-pusher-dev mertis-push-gateway")
            ...
            client.signals
                .sink { [weak self, weak client] signal in
                    Task { @MainActor in
                        ...
                        case .nativeDirectCallTrustDiagnostics(let request):
                            let diagnostic: DirectCallPeerTrustDiagnostic
                            if let flowCoordinator = self?.userSessionFlowCoordinator {
                                diagnostic = await flowCoordinator.nativeDirectCallPeerTrustDiagnostic()
                            } else {
