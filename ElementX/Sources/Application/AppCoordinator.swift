...
class AppCoordinator: AppCoordinatorProtocol, AuthenticationFlowCoordinatorDelegate, NotificationManagerDelegate, SecureWindowManagerDelegate {
    ...
    init(appDelegate: AppDelegate) {
        ...
        MXLog.info("App Store line build: callkit-end-on-hangup unstick-handset audio-only no-video-button outgoing-hangup-callkit lock-answer-no-wait skip-stale-on-push earpiece-voicechat no-audio-session-loop reassert-voicechat-category matrixrtc-presence-incoming capability-retry membership-timeout keep-call-on-dispatch-fail wait-membership-before-prepare http-status-log opaque-call-handle presence-requires-remote keep-audio-overlay dispatch-401-retry leave-webrtc-audio-session no-native-category virtual-earpiece-default pushkit-token-hex sandbox-apns-activation message-pusher-dev mertis-push-gateway pushkit-callkit-first")
