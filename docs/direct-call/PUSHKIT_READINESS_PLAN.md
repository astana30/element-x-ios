...
Status: 2.47C implements the DEBUG-only controlled real media credentials request from the PushKit-controlled Answer path after authenticated pending metadata fetch. The proof records only redacted credentials status and token/URL receipt classes; no raw call ID, room ID, peer ID, call handle, token, URL, request body, media connection, LiveKit join, microphone/camera permission request, Matrix event emission, or production background behavior is implemented by this document. Physical 2.47C proof is pending.
