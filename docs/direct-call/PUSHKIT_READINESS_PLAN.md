...
Status: 2.47B1 adds the safe internal request-metadata handoff needed before a real media credentials request. The foreground accept path records only redacted `DirectCallSession` metadata presence booleans and safe direction/intent classes; no raw call ID, room ID, peer ID, call handle, token, URL, request body, media connection, LiveKit join, Matrix event emission, or production background behavior is implemented by this document.
