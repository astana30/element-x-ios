...
Status: 2.43U retried staging SSH/network deploy path recovery using the corrected port 71 path, but access still times out before authentication or host-key negotiation. PushKit registration remains disabled by default, and no real app-runtime token upload, APNs registration, durable token persistence, VoIP push delivery, media, or production background behavior is implemented by this document.
...
| Server token registration endpoint | locally verified; staging deploy blocked | 2.43Q adds `POST /_matrix/client/unstable/kz.salemx.direct_call/pushkit/token`, auth-gated through the existing Matrix bearer validator and tested with synthetic tokens only. 2.43R validates local fake-token app/server registration against the changed server app with redacted `http_success` and `registered` proofs. 2.43S pre-deploy checks passed, but SSH to staging timed out before deployment. 2.43T confirmed the local staging SSH alias and identity file are present. 2.43U confirmed the alias resolves to port `71`, but the port 71 path still times out before auth/host-key negotiation; live staging still returns `404` for the token route. The endpoint does not durably persist tokens, does not log raw tokens, and does not send VoIP pushes. | Restore network/firewall/VPN/bastion access to the port 71 staging alias, then deploy the server endpoint and rerun staging synthetic-token smoke before storage/provider work. | Without staging deployment, durable storage, and invalidation, a VoIP provider still cannot target native direct-call devices. |
