...
Status: 2.44B implements and deploys controlled server-side PushKit token persistence, but the physical real-token persistence smoke is blocked by unavailable CoreDevice phones. PushKit registration remains disabled by default, and no standard APNs registration, APNs provider request, VoIP push delivery, media, or production background behavior is implemented by this document.
...
| Server token registration endpoint | controlled persistence deployed; physical persistence smoke blocked | 2.43Q adds `POST /_matrix/client/unstable/kz.salemx.direct_call/pushkit/token`, auth-gated through the existing Matrix bearer validator and tested with synthetic tokens only. 2.43R validates local fake-token app/server registration against the changed server app with redacted `http_success` and `registered` proofs. 2.43W copied the endpoint runtime files to staging. 2.43Y verifies the service is active after restart and localhost token registration returns unauthenticated `401`. 2.43Z maps the public token route through the reverse proxy; public unauthenticated token registration is now `401`, and synthetic-token public staging registration passes with `http_success` and `registered`. 2.44A uses a DEBUG/manual physical smoke to upload one real PushKit token to the staging endpoint with redacted `http_success` and `registered` proof. 2.44B adds a file-backed server token store with hashed identity keys and restricted file permissions, deploys it to staging, and verifies route safety, but the physical token persistence smoke is blocked by unavailable iPhones. The API does not return tokens, logs remain redacted, and no VoIP push is sent. | Rerun the controlled physical upload after device availability is restored; then design invalidation. Do not move to VoIP push delivery until storage verification, invalidation, and provider readiness are explicitly scoped. | Persistence code is deployed, but real-token persisted proof is not complete until the physical smoke reruns successfully. |
...
- Storage is file-backed at `state/pushkit-tokens.json` under the service working directory.
...
The controlled physical real-token persistence smoke was not rerun because physical iPhones were unavailable through CoreDevice. Redacted blocker: `physical_device_unavailable`.
