...
Status: 2.43Y confirms the restarted staging service has the token route active on localhost, but the public staging token route still returns `404`. PushKit registration remains disabled by default, and no real app-runtime token upload, APNs registration, durable token persistence, VoIP push delivery, media, or production background behavior is implemented by this document.
...
| Server token registration endpoint | locally verified; active on staging localhost; public route missing | 2.43Q adds `POST /_matrix/client/unstable/kz.salemx.direct_call/pushkit/token`, auth-gated through the existing Matrix bearer validator and tested with synthetic tokens only. 2.43R validates local fake-token app/server registration against the changed server app with redacted `http_success` and `registered` proofs. 2.43W copied the endpoint runtime files to staging. 2.43Y verifies the service is active after restart and localhost token registration returns unauthenticated `401`, but public staging still returns `404` for the token route. The endpoint does not durably persist tokens, does not log raw tokens, and does not send VoIP pushes. | Update the public staging reverse-proxy/path mapping for the token route, then rerun route safety and synthetic-token staging smoke before storage/provider work. | Until the public route maps to the service, app/server staging smoke cannot use the deployed endpoint. |
