...
Status: 2.43R validates a local fake-token app/server registration smoke against the changed 2.43Q server endpoint. PushKit registration remains disabled by default, and no real app-runtime token upload, APNs registration, durable token persistence, VoIP push delivery, media, or production background behavior is implemented by this document.
...
| Server token registration endpoint | locally verified with fake token | 2.43Q adds `POST /_matrix/client/unstable/kz.salemx.direct_call/pushkit/token`, auth-gated through the existing Matrix bearer validator and tested with synthetic tokens only. 2.43R validates local fake-token app/server registration against the changed server app with redacted `http_success` and `registered` proofs. The endpoint does not durably persist tokens, does not log raw tokens, and does not send VoIP pushes. Live staging still needs deployment before this route returns anything other than `404` there. | Add durable storage/invalidation design or a separately authorized deployment validation; do not upload a real runtime token until explicitly scoped. | Without durable storage and invalidation, a VoIP provider still cannot target native direct-call devices. |
