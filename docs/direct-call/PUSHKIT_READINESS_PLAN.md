...
Status: 2.47A10 keeps the real-invite background chain narrowed to a CallKit auto-End blocker. Local CallKit-only Answer delivery works. The latest one-shot physical background proof reached APNs `sandbox_success`, PushKit receipt, CallKit report `reported`, report completion, PushKit completion in `<100ms`, foreground app state at receipt/report/action, retained provider/delegate/active UUID, valid generic audio-only CallKit update/config, and then `callkit_first_action_kind=end` more than `2000ms` after PushKit completion. The operator did not observe an answerable UI surface. Local app End request, provider invalidation, report-ended, and controlled timeout before Answer were all false. Current blocker is `background_callkit_end_before_operator_action`. No standard APNs registration, production APNs, repeated push, real media credentials request, media connection, LiveKit join, Matrix event emission, or production background behavior is implemented by this document.
