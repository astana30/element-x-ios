...
Status: 2.47A7 keeps the real-invite background chain narrowed to a CallKit End-before-Answer blocker. The latest one-shot physical proof reached APNs `sandbox_success`, PushKit receipt, CallKit report `reported`, report completion, retained provider/delegate/active UUID, and then `callkit_first_action_kind=end` with `callkit_first_action_after_report_ms_bucket=>2000ms`. Local app End request, provider invalidation, report-ended, and controlled timeout before Answer were all false. 2.47A7 adds only redacted operator-intent/timing proof fields and Internal diagnostics controls for the next one-shot physical validation. Current blocker remains `system_or_user_end_before_answer`. No standard APNs registration, production APNs, repeated push, real media credentials request, media connection, LiveKit join, Matrix event emission, or production background behavior is implemented by this document.
