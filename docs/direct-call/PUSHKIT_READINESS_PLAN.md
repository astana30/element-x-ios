...
Status: 2.44B1 validates controlled physical real-token upload into the persisted staging token store. The proof is fully redacted, iOS local persistence remains disabled, and PushKit registration remains disabled by default. No standard APNs registration, APNs provider request, VoIP push delivery, media, or production background behavior is implemented by this document.
