...
Status: 2.44A validates a controlled physical-device PushKit token upload smoke through an explicit DEBUG/manual path. PushKit registration remains disabled by default, and no standard APNs registration, durable token persistence, VoIP push delivery, media, or production background behavior is implemented by this document.
