...
Status: resumed 2.43W confirms SSH publickey auth on port 71 works and the endpoint runtime files were copied, but service restart is blocked by restart authorization. PushKit registration remains disabled by default, and no real app-runtime token upload, APNs registration, durable token persistence, VoIP push delivery, media, or production background behavior is implemented by this document.
...
| Direct service status | verified active, restart blocked | Resumed 2.43W verified `salemx-call-service` is active, but restart is blocked by service restart authorization. | Restart only the call service through an approved privileged path before claiming endpoint deployment. | Active status alone does not prove the copied files are loaded. |
