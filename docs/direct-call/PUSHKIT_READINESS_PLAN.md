...
Status: 2.47B5 adds the safe authenticated pending-metadata fetch handoff needed before a real media credentials request from the PushKit-controlled Answer path. The proof records only redacted fetch status and metadata presence/safe direction/intent classes; no raw call ID, room ID, peer ID, call handle, token, URL, request body, media connection, LiveKit join, Matrix event emission, or production background behavior is implemented by this document.
