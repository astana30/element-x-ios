...
Status: 2.43Z exposes the staging token-registration route publicly through the reverse proxy and validates synthetic-token staging registration. PushKit registration remains disabled by default, and no real app-runtime token upload, APNs registration, durable token persistence, VoIP push delivery, media, or production background behavior is implemented by this document.
