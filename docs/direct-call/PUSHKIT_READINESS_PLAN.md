...
Status: 2.43X confirms staging service restart activation is blocked by sudo password policy. PushKit registration remains disabled by default, and no real app-runtime token upload, APNs registration, durable token persistence, VoIP push delivery, media, or production background behavior is implemented by this document.
...
| Server token registration endpoint | locally verified; staging files copied but restart-blocked; manual package documented | 2.43Q adds `POST /_matrix/client/unstable/kz.salemx.direct_call/pushkit/token`, auth-gated through the existing Matrix bearer validator and tested with synthetic tokens only. 2.43R validates local fake-token app/server registration against the changed server app with redacted `http_success` and `registered` proofs. 2.43S pre-deploy checks passed, but SSH to staging timed out before deployment. 2.43T confirmed the local staging SSH alias and identity file are present. 2.43U confirmed the alias resolves to port `71`, but the port 71 path still timed out before auth/host-key negotiation. 2.43V documents a manual deploy runbook listing the runtime files and validation steps. 2.43W confirms port `71` connectivity is open after fail2ban unban, and resumed 2.43W verifies SSH publickey auth and copies the two endpoint runtime files. 2.43X confirms restart is blocked because sudo requires a password, so live staging still returns `404` for the token route. The endpoint does not durably persist tokens, does not log raw tokens, and does not send VoIP pushes. | Restart only `salemx-call-service` through an approved privileged path, then rerun route safety and synthetic-token staging smoke before storage/provider work. | Without service restart, staging keeps serving the previous process and the token route remains missing. |
...
| Direct service status | verified active, restart blocked | 2.43X verified `salemx-call-service` is active, but restart is blocked because sudo requires a password. | Restart only the call service through an approved privileged path before claiming endpoint deployment. | Active status alone does not prove the copied files are loaded. |
