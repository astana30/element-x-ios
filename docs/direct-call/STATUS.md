...
After 2.47C, the PushKit-controlled Answer path uses the authenticated pending-metadata handoff to request real media credentials through the existing `DirectCallLiveKitTokenProvider` boundary, then records only redacted proof fields. 2.47C2 fixed the public token-route 404 by using the auth-gated foreground-signaling token alias. 2.47C5 classified the pending metadata fetch blocker as `403 M_FORBIDDEN`. 2.47C6 fixes the server receiver-device binding mismatch: pending metadata is now bound to the exact receiver device only when that device token is also the latest PushKit token APNs will use; otherwise the metadata is bound to the receiver account while unrelated users remain forbidden. The next physical proof should verify pending metadata fetch success and then re-enter the no-connect credentials boundary. This remains no-connect: no media connection, LiveKit join, microphone/camera permission request, Matrix event emission, or full call flow is run.
