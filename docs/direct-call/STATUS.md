...
2.47C controlled real media credentials request is physically closed as a no-connect proof. Real invite/APNs/PushKit/CallKit Answer, authenticated pending metadata fetch, foreground metadata handoff, and the existing `DirectCallLiveKitTokenProvider` credentials request all succeeded with redacted token/URL receipt proof. Cleanup proof recorded credentials cleanup as cleared. Safety remained intact: no media connection, LiveKit join, microphone/camera permission request, Matrix event emission, or full call flow was run. Next phase is 2.47D credentials cleanup / expiry proof, not real call flow.
