...
After 2.47C, the PushKit-controlled Answer path uses the authenticated pending-metadata handoff to request real media credentials through the existing `DirectCallLiveKitTokenProvider` boundary, then records only redacted proof fields. 2.47C2 fixed the public token-route 404 by using the auth-gated foreground-signaling token alias. 2.47C6 fixed the pending metadata receiver-device binding mismatch and physical proof then reached metadata fetch success. 2.47C7 fixes the no-connect token eligibility rejection for incoming receiver requests by keeping the receiver allowlist gate while allowing the room-validated caller peer for incoming direction. The next physical proof should verify redacted token issuance. This remains no-connect: no media connection, LiveKit join, microphone/camera permission request, Matrix event emission, or full call flow is run.
