...
After 2.47C, the PushKit-controlled Answer path uses the authenticated pending-metadata handoff to request real media credentials through the existing `DirectCallLiveKitTokenProvider` boundary, then records only redacted proof fields. 2.47C2 fixed the public token-route 404 by using the auth-gated foreground-signaling token alias. 2.47C6 fixed the pending metadata receiver-device binding mismatch and physical proof then reached metadata fetch success. 2.47C7 fixed the incoming receiver eligibility policy. 2.47C8 fixes the persistent deployment `eligibilityRejected` caused by the staging env using the legacy `SALEMXNATIVE_AUDIO_ELIGIBILITY_ENABLED` switch spelling; the service now treats that explicit legacy switch as equivalent to the canonical switch while preserving allowlist requirements. The next verification should be a direct credentials test first, not APNs. This remains no-connect: no media connection, LiveKit join, microphone/camera permission request, Matrix event emission, or full call flow is run.
