...
After 2.47C, the PushKit-controlled Answer path uses the authenticated pending-metadata handoff to request real media credentials through the existing `DirectCallLiveKitTokenProvider` boundary, then records only redacted proof fields. 2.47C2 fixed the public token-route 404 by using the auth-gated foreground-signaling token alias. The latest physical proof then blocked earlier at `pending_metadata_fetch_http_failure_redacted`; public route probes show the pending-metadata path is deployed and auth-gated, and server tests prove same-process receiver fetch succeeds. 2.47C5 adds redacted pending-metadata fetch HTTP classification so the next proof can distinguish auth, lookup, route, or server failure without exposing identifiers. This remains no-connect: no media connection, LiveKit join, microphone/camera permission request, Matrix event emission, or full call flow is run.
