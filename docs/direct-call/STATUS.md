...
After 2.49Z-ZhanielkaSenderAuthorizedMetadataBoundary — 2.49Y confirmed that the receiver/APNs pending metadata reference is not the right source for sender-side credentials: receiver fetch succeeds, sender fetch is auth-rejected, sender credentials are not requested, and all media/LiveKit/permission/event/full-flow guards stay closed. 2.49Z adds a separate sender-authorized metadata reference/source on real invite success and teaches the DEBUG atomic handoff file to pass only that sender-authorized source to iPhone Жанелька. The next phase is `2.49Z-ZhanielkaSenderAuthorizedMetadataBoundary — one-shot fresh invite with sender-authorized metadata source, stop before media connect/LiveKit`.
...
- 2.49Z-ZhanielkaSenderAuthorizedMetadataBoundary provides the minimal sender-authorized metadata source required by 2.49Y:
  ```text
  sender_authorized_metadata_source_available=<runtime>
  ...
  sender_pending_metadata_raw_identifiers_logged=false
  ```
  - The call-service stores a receiver/APNs metadata reference and a separate sender-only metadata reference. The receiver endpoint rejects sender-only references, and the sender endpoint rejects receiver/APNs references.
  - The real invite response exposes the sender-only reference as `sender_authorized_metadata_reference` plus redacted diagnostics; raw metadata and identifiers remain out of logs/docs.
  - The DEBUG atomic sender handoff-and-trigger file now accepts `sender_authorized_metadata_reference` with marker `2.49Z`; this arms the sender runtime with `sender_authorized_file_handoff_redacted`.
  - Sender credentials remain behind successful sender metadata fetch. The sender no-media trigger still stops before media connect/LiveKit/microphone/camera/Matrix/full flow.
  - The helper path is `/tmp/salemx_2_49z_zhanielka_sender_authorized_metadata_boundary.command`.
...
- 2.48Z-ParticipantObserverPropagationRepair closes Retry22 as sender runtime join + receiver connected-window overlap success / participant observer propagation timeout triage, then repairs receiver participant observation without APNs/connect:
  ...
  - New blocker:
    ```text
    receiver connected-window overlap is proven, but receiver LiveKit participant observer propagation did not observe a remote participant
    ```
  - Repair proof fields include `participant_observer_propagation_repair_*`, retained/connected-room binding booleans, callback/snapshot observation booleans, snapshot count bucket, redacted identity-filter status, and specific propagation classifications.
  - Safety preserved: no APNs in this repair, no production APNs, no `dev/invite`, no physical connect/LiveKit join, no microphone/camera permission, no video, no Matrix event emission, no full flow, and no raw token/JWT/auth header/APNs payload/invite body/LiveKit URL/room/call/user/device/pending-metadata logging.
...
- 2.48Z-ReceiverConnectedWindowRetentionRepair closes Retry21 as sender runtime join bridge success / receiver connected window closed before sender signal triage, then repairs receiver lease retention without APNs/connect:
  - Retry21 physical classification:
    ```text
    APNs_sent=true
    background_apns_push_result=sandbox_success
    physical_voip_push_received=true
  ...
  - New blocker:
    ```text
    receiver connected window did not overlap the sender connected signal
    ```
  - Repair proof fields include `receiver_connected_window_retention_repair_*`, lease-active-before/after/at-signal booleans, before/after sender-signal disconnect booleans, post-overlap participant observation booleans, and retained-window classification buckets.
...
- 2.48Z-ReceiverSenderConnectedSignalRepair adds the no-APNs/no-connect repair for the Retry19 blocker:
  ...
  - Receiver proof can consume the signal before disconnect through `/direct-call/sender-connected-signal-handoff` and classify:
    ...
    ```
  - Redacted buckets now distinguish `receiver_connected_window_retained_until_sender_signal_redacted`, `sender_connected_signal_missing_redacted`, `sender_connected_signal_after_receiver_disconnect_redacted`, `sender_connected_signal_correlation_mismatch_redacted`, `receiver_window_closed_before_sender_signal_redacted`, `receiver_connected_window_overlap_observed_redacted`, and `participant_observation_timeout_after_overlap_redacted`.
...
- 2.48Z-RemoteParticipantObservationRuntimeActivationRepair closes Retry14 as real sender runtime join success / receiver observation window not activated / remote participant not observed triage, then wires the already-built observation timing repair into the actual receiver controlled-connect runtime path without APNs/connect:
  ...
  - Repair behavior:
    ...
    receiver observer is marked active before sender terminal propagation
    receiver cleanup remains deferred until observation terminal
    sender runtime terminal state is propagated to receiver correlation proof
  ...
  - No APNs, production APNs, repeated APNs, `dev/invite`, physical media connect, physical LiveKit join, microphone/camera permission, video, Matrix event emit, full call flow, raw identifier logging, or signing/project file edit was performed.
  - Next phase: `2.48Z-Physical2-Retry15 — one-shot real sender join with activated receiver observation window`.
...
- 2.48Z-Physical2-Retry9-SenderTimelineTimeoutTriage closes the one-shot two-physical-device sender SDK timeline proof as safe sender SDK timeline timeout / remote participant not observed triage, not remote-audio success:
  ...
  - Receiver path succeeded once:
    ...
    livekit_join_result=success_redacted
    ```
  - Sender trigger orchestration reached a terminal sender timeline classification:
    ```text
    sender_join_trigger_orchestration_present=true
    sender_join_trigger_orchestration_apns_success_seen=true
    sender_join_trigger_orchestration_receiver_answer_seen=true
  ...
  - Conclusion:
    ...
    ```
  - Next phase: `2.48Z-SenderSDKConnectTimeoutRepair — diagnose sender SDK connect timeout after orchestrated trigger, no APNs/connect`.
...
- 2.48Z-Physical2-Retry4-SenderTransportFailedTriage closes the one-shot two-physical-device sender join failure-bucket proof as safe sender transport-failed triage, not remote-audio success:
  ...
  - Receiver metadata, credentials, one controlled connect, and receiver LiveKit join succeeded:
    ...
    livekit_join_result=success_redacted
    ```
  - Sender readiness survived into runtime and Answer; sender-side join activation triggered and consumed exactly once:
    ```text
    ...
    sender_side_livekit_join_activation_repeated=false
    ```
  - Sender join failure diagnostics classified the first sender-side join attempt:
    ```text
    sender_side_livekit_join_requested=true
    sender_side_livekit_join_result=failed_redacted
    sender_side_livekit_join_error_bucket=transport_failed_redacted
  ...
  - Conclusion:
    ...
    No full call flow.
    ```
  - Next phase: `2.48Z-SenderJoinTransportFailureRepair — investigate sender-side LiveKit transport failure before any APNs retry, no APNs/connect`.
...
- 2.48Z-Physical2-Retry3-SenderJoinFailedTriage closes the one-shot two-physical-device sender join activation / remote participant proof as safe sender-join-failed triage, not remote-audio success:
  ...
  - Receiver metadata, credentials, one controlled connect, and receiver LiveKit join succeeded:
    ...
    livekit_join_result=success_redacted
    ```
  - Sender readiness survived into runtime and Answer, then sender-side join activation triggered and consumed exactly once:
    ```text
    ...
    sender_side_livekit_join_activation_repeated=false
    ```
  - Sender-side LiveKit join did not succeed, so receiver remote participant/audio/liveness was not observed:
    ```text
    sender_side_livekit_join_requested=true
    sender_side_livekit_join_result=failed_redacted
...
- 2.48Z-Physical2-Retry2-SenderNotJoinedTriage safely classifies the one-shot two-physical-device sender readiness / remote participant proof as sender-not-joined, not remote-audio success:
  ...
  - APNs audit:
    ...
    ```
    The local phase marker proves at least one sandbox APNs success. The terminal transcript reported an `APNs_sent=true` block before a restored session and then another `SEND_2_48Z_PHYSICAL2_RETRY2` helper confirmation. Because the retained local marker does not distinguish whether those were the same helper session or two sends, the close-out records possible repeated APNs explicitly. No APNs was sent during this close-out and no further APNs may be sent for this phase.
  - Physical proof generation:
    ```text
    proof_generation=generation_18
    physical_voip_push_received=true
  ...
  - Receiver metadata, credentials, and one controlled connect succeeded:
    ...
    ```
  - Sender readiness reached runtime and survived Answer, but sender-side join remained unarmed/not requested:
    ```text
...
- 2.48Y-RemotePeerContextHandoffRepair carries redacted simulator peer readiness into runtime proof without APNs/connect:
  - Added DEBUG/test-controlled remote peer context handoff state and URL hook:
    ...
    remote_peer_context_handoff_debug_only=true
    remote_peer_context_handoff_raw_identifiers_logged=false
    remote_peer_context_handoff_blocks_success_without_context=true
    remote_peer_context_handoff_classifies_missing_remote_participant=true
    remote_peer_context_handoff_classifies_simulator_limitation=true
    ```
  - Future simulator-assisted runtime proof can record:
    ```text
    remote_peer_context_handoff_source=debug_hook_redacted
    remote_peer_context_handoff_armed_before_apns=true
    remote_peer_context_handoff_received_by_runtime=true
    ...
    remote_audio_liveness_limitation=simulator_assisted_redacted
    ```
  - Missing context is explicit non-success:
    ```text
    remote_peer_context_handoff_received_by_runtime=false
  ...
  - Safety remains closed by default:
    ...
    real_call_flow_started=false
    ```
  - This repair did not send APNs, run production APNs, repeat APNs, use `dev/invite`, retry media connect, retry LiveKit join, request microphone/camera permission, enable video, emit Matrix events, start full call flow, reset/re-arm the physical one-shot connect hook, or modify project/signing files.
...
- 2.48W controlled disconnect/end-call cleanup review is complete:
  ...
  - Media/LiveKit activity remained the single Physical8 audio-only attempt:
    ...
    livekit_connect_audio_invoked=true
    ```
  - Cleanup was requested and marked ended, but CallKit End action proof was incomplete:
    ```text
...
- 2.48T-Physical8 physically proves the one-shot Answer -> metadata reference -> credentials -> first controlled audio-connect path:
  ...
  - The physical proof generation was `generation_14`.
  - PushKit, CallKit report completion, and CallKit Answer all completed:
    ```text
    physical_voip_push_received=true
    pushkit_callback_invoked=true
    pushkit_payload_kind=real_invite_controlled
    ...
    callkit_answer_action_fulfilled=true
    ```
  - The pending metadata reference repair and metadata fetch succeeded:
    ```text
    pending_metadata_reference_present=true
    ...
    foreground_pending_call_metadata_handoff_observed=true
    ```
  - Media credentials and the controlled first attempt succeeded through the DEBUG-only one-shot audio path:
    ```text
    media_credentials_requested=true
    media_credentials_request_authorized=true
    media_credentials_result=success_redacted
    ...
    controlled_connect_first_attempt_error_bucket=none
    ```
  - Runtime scope stayed audio-only and closed outside the one attempt:
    ```text
    media_connect_requested=true
    media_connect_attempted=true
    livekit_join_requested=true
...
- 2.48T-Physical6-MetadataCredentialsBoundaryRepair fixes the Answer -> metadata/credentials boundary without running a physical call:
  ...
  - After real CallKit Answer, the proof can now show the metadata boundary handoff and fetch trigger:
    ...
    pending_metadata_fetch_requested=true
    ```
  - If the pending metadata reference is missing after Answer, the old generic credentials boundary state is replaced with a precise no-credentials state:
    ```text
    pending_metadata_fetch_result=blocked_redacted
    pending_metadata_fetch_failure_reason=missing_reference_after_answer
    media_credentials_requested=false
    ...
    blocked_reason=pending_metadata_missing_after_answer_no_credentials
    ```
  - If pending metadata succeeds, the credentials boundary is permitted:
    ```text
    pending_metadata_fetch_result=success_redacted
    pending_metadata_fetch_http_status_bucket=2xx
    pending_metadata_fetch_errcode=none
    ...
    media_credentials_request_authorized=true
    ```
  - The repair proof fields are present and redacted:
    ```text
    metadata_credentials_boundary_repair_present=true
    ...
    metadata_credentials_boundary_repair_raw_credentials_logged=false
    ```
  - The Physical6 hook is not consumed before metadata/credentials; it can only be consumed after Answer plus metadata success plus credentials eligibility reaches the controlled first-attempt gates.
...
- 2.48T-CallKitSurfaceRepair fixes/classifies the CallKit surface gap before any APNs retry:
  ...
  - The VoIP PushKit proof now includes redacted CallKit surface repair diagnostics:
    ```text
    callkit_surface_repair_present=true
    ...
    callkit_surface_repair_no_media_connect_on_no_answer=true
    ```
  - If CallKit report completion is observed, the proof can record:
    ```text
    callkit_report_result=reported
    callkit_report_completion_observed=true
    pushkit_completion_called=true
    ```
  - If report completion remains missing, the repair watchdog classifies the result instead of leaving the path silently pending:
    ```text
    ...
    ```
  - The answerable-window safety path records deterministic PushKit completion states:
    ```text
    callkit_surface_repair_pushkit_completion_safety_result=completed_after_first_action
    callkit_surface_repair_pushkit_completion_safety_result=completed_after_answerable_window_timeout
    callkit_surface_repair_pushkit_completion_safety_result=completed_after_report_completion
    ```
  - Default no-answer safety remains closed:
    ```text
    pending_metadata_fetch_requested=false
    media_credentials_requested=false
    media_connect_requested=false
    ...
    real_call_flow_started=false
    ```
  - Tests/source guards cover pending report retention, report completion success/failure classification, PushKit completion safety, no direct answer bypass, no metadata/credentials/connect without Answer, and the existing controlled bridge path.
