...
Next, continue with `2.40C — native audio signing and entitlement readiness audit` to verify Push Notifications capability, APNs `aps-environment`, VoIP background mode, provisioning, Apple Developer account capability, physical-device availability, and push gateway prerequisites before any CallKit/PushKit/APNs implementation begins. Broad internal rollout, production/public rollout, unsupervised dogfood, participant/device expansion, CallKit/push implementation in this phase, missed-call UX, video, session restoration, and global activation remain blocked.
